...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
        ...
        if query.startswith("**Catuserbot") and event.query.user_id == bot.uid:
            buttons = [
                (
                    ...
                    Button.url("DEV", "t.me/shashankxD"),
                )
            ]
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:
            ...
            reply_pop_up_alert += (
                "Use .unload {} to remove this plugin ©Akito".format(plugin_name)
            )
            try:
                await event.answer(reply_pop_up_alert, cache_time=0, alert=True)
            except BaseException:
            ...
            reply_pop_up_alert = "chla ja madrchod"
            await event.answer(reply_pop_up_alert, cache_time=0, alert=True)
    ...
    async def on_plug_in_callback_query_handler(event):
        if event.query.user_id == bot.uid:
            ...
            reply_pop_up_alert = "chla jaa madrchod "
            await event.answer(reply_pop_up_alert, cache_time=0, alert=True)
